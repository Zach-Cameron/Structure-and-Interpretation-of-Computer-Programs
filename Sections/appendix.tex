...
\subsubsection{Propositional Expressions and Predicates}
...
\subsubsection{Expressions for Recursive Functions}
...
\subsubsection{A Class of Symbolic Expressions}
...
\subsubsection{Functions of S-expressions and the Expressions That Represent Them}
...
\subsubsection{The Elementary S-functions and Predicates}
...
\subsubsection{Recursive S-functions}
...
\begin{exmp}
    ...
    Therefore:
    \begin{itemize}
        \item sublis[x;y] = (A $\cdot$ ((A, B) $\cdot$ (B, C))) = (A $\cdot$ ((A, B) $\cdot$ (B $\cdot$ (C $\cdot$ NIL)))) = (A, (A,B), B, C)
    \end{itemize}
\end{exmp}

\subsubsection{Representation of S-Functions by S-Expressions}

Rules for translating M-expressions into S-expressions:
\begin{enumerate}
    \item If $\mathcal{E}$ is an S-expression, then $\mathcal{E}^{*}$ is (QUOTE, $\mathcal{E}$).
    \item Variables and function names that were represented by strings of lowercase letters are translated to the corresponding strings of the corresponding uppercase letters. E.g.: $\text{car}^{*}$ is CAR and $\text{subst}^{*}$ is SUBST.
    \item A form $f[e_{1};\ldots;e_{n}]$ is translated to $(f^{*}, e_{1}^{*},\ldots,e_{n}^{*})$. E.g.: $\text{cons[car[x];cdr[x]]}^{*}$ is (CONS, (CAR, X), (CDR, X)).
    \item ${[p_{1} \rightarrow e_{1}; \ldots; p_{n} \rightarrow e_{n}]}^{*}$ is (COND, $(p_{1}^{*}, e_{1}^{*})$, $\cdots$, $(p_{n}^{*} \cdot e_{n}^{*})$).
    \item ${\lambda[[x_{1}; \ldots; x_{n}]; \mathcal{E}]}^{*}$ is (LAMBDA, ($x_{1}^{*}$, $\cdots$, $x_{n}^{*}$), $\mathcal{E}^{*}$).
...
\begin{exmp}
    The M-expression:
    \begin{center}
        label[subst; $\lambda$[[x; y; z]; [atom[z] $\rightarrow$ [eq[y; z] $\rightarrow$ x; T $\rightarrow$ z]; T $\rightarrow$ cons[subst[x; y; car[z]]; subst[x; y; cdr[z]]]]]]   
    \end{center}
...
\begin{note}
    What this function does: Unfurl the tree of z. For every leaf, if the leaf is equivalent to y then replace the leaf with x, otherwise leave the leaf alone. Return the resulting S-expression.
\end{note}

\subsubsection{The Universal S-Function \texttt{apply}}

\begin{defn}[\idx{apply[$f$; $args$]}]
    apply[$f$; $args$] = eval[cons[$f$; appq[$args$]]; NIL]

    Given S-expression $f$ representing S-function $f'$, and given $args$ is a list of arguments of the form $(arg_{1},\ldots,arg_{n})$ where $arg_{1},\ldots,arg_{n}$ are arbitrary S-expressions: apply[$f$; $args$] is defined to be equivalent to $f'[arg_{1},\ldots,arg_{n}]$.
\end{defn}
